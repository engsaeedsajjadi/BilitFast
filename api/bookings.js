...
module.exports = async (req, res) => {
  ...
  try {
    if (action === 'save') {
      ...
      const paymentUrl = String(body.paymentUrl || '').slice(0, 1000);
      const bookingData = {
        ...
        passenger_name: String(booking.passenger_name || booking.passengerName || '').slice(0, 200),
      };
    ...
    if (action === 'notify-payment') {
      ...
      if (!rec.payment_url) return res.status(200).json({ ok: false, error: 'این رزرو لینک پرداخت ندارد.' });
      const r = await notify.sendPaymentLinkToUser(user, rec.booking || {}, rec.payment_url);
      db.update('bookings', rec.id, {
        payment_notification: {
          status: r.ok ? 'sent' : (r.skipped || 'failed'),
          results: r.results || [],
          sent_at: r.ok ? Date.now() : null,
          resent_at: Date.now(),
        },
      });
      return res.status(200).json({ ok: r.ok, paymentNotification: r });
    }
