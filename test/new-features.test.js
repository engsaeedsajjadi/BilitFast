...
/* ۱) تاب‌آوری شبکه: خطای اتصال «گذرا» است و جستجو را متوقف نمی‌کند.
 *
 * سابقاً این تست انتظار fatal=true داشت («توقف هوشمند»). در عمل مشخص شد
 * سامانه صفیر ریل مرتباً قطعی کوتاه و ری‌ست اتصال دارد، و توقف با اولین
 * خطا باعث می‌شد جستجو پس از دو-سه تلاش بایستد و کاربر ظرفیت را از دست
 * بدهد. حالا خطای شبکه transient است و حلقه با عقب‌نشینی ادامه می‌دهد؛
 * توقف فقط پس از چند خطای پیاپی رخ می‌دهد (لایه کلاینت). */
...
(async () => {
  ...
  test('خطای شبکه → گذرا و بدون fatal (جستجو ادامه می‌یابد)',
    r.ok === false && !r.fatal && r.transient === true);
  test('راهنمای خطای شبکه به محیط اجرا وابسته است',
    typeof r.hint === 'string' && !/Vercel/i.test(r.hint));
