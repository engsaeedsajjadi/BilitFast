...
async function readChromeCookies() {
  ...
  trace.log('chrome', 'فایل‌های کوکی پیدا شد', {
    count: files.length,
    files: files.map(trace.safePath),
  });
  if (!files.length) {
    trace.log('chrome', 'نتیجه', {
      ok: false,
      reason: 'هیچ مرورگر مبتنی بر کرومیوم (کروم/اج/بریو) روی این دستگاه نیست',
    });
    return [];
  }
  ...
  trace.log('chrome', 'کلید رمزگشایی', {
    platform, mode, ok: !!key,
    error: key ? undefined : (keyError || 'کلید در دسترس نیست'),
  });
  if (!key) {
  ...
  for (const file of files) {
    ...
    try {
      ...
      let domainHits = 0; let decryptFails = 0; let localAppBound = 0;
      trace.log('chrome', 'خواندن جدول کوکی', {
        file: trace.safePath(file), totalRows,
      });
      if (res.length && res[0].values) {
        for (const row of res[0].values) {
          ...
          try {
            ...
            if (err && err.code === 'APP_BOUND') { appBound++; localAppBound++; continue; }
            throw err;
          }
      ...
      trace.log('chrome', 'کوکی‌های دامنه صفیر ریل', {
        file: trace.safePath(file),
        matched: domainHits,
        decryptFailed: decryptFails,
        appBound: localAppBound,
      });
    } catch (e) {
      trace.logError('chrome', 'خطا در خواندن فایل', e);
      continue;
    } finally {
  ...
  const outNames = trace.cookieNames(Array.from(found.values()));
  trace.log('chrome', 'نتیجه', {
    ok: found.size > 0,
    count: found.size,
    names: outNames,
    appBound,
    hasSession: outNames.some((n) => /^PHPSESSID$/i.test(n)),
  });
