...
// حالت «فقط رقم». طبق نمونه‌های واقعی، کپچای صفیر ریل الفبا+ارقام است
// (بدون 0/O/l/I) و force_digits در تنظیمات غیرفعال است؛ این گزینه صرفاً برای
// سازگاری با کپچاهای عددیِ احتمالی در آینده نگه داشته شده است.
...
// مدل الفبایی آموزش‌دیده روی کاراکترهای واقعی سایت (ترجیحاً CNN با tfjs، فال‌بک MLP)
...
function loadCharModel() {
  ...
  // ۱) CNN (دقیق‌تر)
  try {
    const cnnPath = path.join(__dirname, '..', CAPTCHA_CFG.char_cnn_path || 'models/char-cnn.json');
    if (fs.existsSync(cnnPath)) {
      const json = JSON.parse(fs.readFileSync(cnnPath, 'utf8'));
      if (json && json.classes && json.weights) {
        const { buildCharCNN, getTf } = require('./cnn');
        const t = getTf();
        const model = buildCharCNN(json.classes.length);
        model.setWeights(json.weights.map((w) => t.tensor(w.data, w.shape)));
        // دمای کالیبراسیون اطمینان (از اعتبارسنجی هنگام آموزش؛ پیش‌فرض ۱)
        const temperature = (json.meta && typeof json.meta.temperature === 'number' && json.meta.temperature > 1e-6)
          ? json.meta.temperature : 1;
        _charModel = { engine: 'cnn', model, classes: json.classes, temperature };
        return _charModel;
      }
    }
  } catch (e) { /* tfjs در دسترس نیست → فال‌بک MLP */ }
  // ۲) فال‌بک MLP
  try {
    ...
    if (fs.existsSync(modelPath)) {
      ...
      if (json && json.classes && json.layers) {
        ...
        _charModel = { engine: 'mlp', model: MLP.fromJSON(json), classes: json.classes, temperature: 1 };
      }
    }
...
/**
 * پیش‌بینی کلاس برای بردارهای کاراکتر با موتور فعال (CNN یا MLP).
 * برای CNN، احتمال‌ها با دمای کالیبره‌شده تعدیل می‌شوند تا اطمینان گزارش‌شده
 * به احتمال واقعی نزدیک‌تر باشد (بدون تغییر رتبه‌بندی کلاس‌ها).
 */
function predictChars(cm, vecs) {
  if (cm.engine === 'cnn') {
    const t = require('./cnn').getTf();
    const flat = new Float32Array(vecs.length * 400);
    vecs.forEach((v, i) => flat.set(v, i * 400));
    const xs = t.tensor4d(flat, [vecs.length, 20, 20, 1]);
    const probsArr = cm.model.predict(xs).arraySync();
    xs.dispose();
    const T = cm.temperature || 1;
    return probsArr.map((probs) => {
      ...
      return { label, conf: p[label] };
    });
  }
  return vecs.map((v) => {
    const { label, probs } = cm.model.predict(Float64Array.from(v));
...
async function solveWithCharModel(buffer) {
  ...
  const vecs = comps.map((comp) => Array.from(normalizeComponent(comp, { size: 20, inner: 16 })));
  const preds = predictChars(cm, vecs);
  let text = '';
  let confSum = 0;
  let minConf = 1;
  const chars = [];
  for (const p of preds) {
    const digit = cm.classes[p.label];
    if (!digit) return null;
    text += digit;
    confSum += p.conf;
    if (p.conf < minConf) minConf = p.conf;
    chars.push({ digit, conf: Math.round(p.conf * 100), source: 'char-' + cm.engine });
  }
...
async function recognize(buffer, worker, whitelist, psm) {
  ...
  try {
    ...
    // حفظ بزرگی/کوچکی حروف: الفبای کپچای واقعی شامل حروف کوچک هم هست
    // (مثل «x» یا «z») و سایت به بزرگی/کوچکی حساس است.
...
function scoreResult(r, forceDigits = FORCE_DIGITS) {
  ...
  // در حالت «فقط رقم»، پاسخ حاوی حروف تقریباً همیشه توهم تِسِرَکت است
  if (forceDigits && !allDigits) score -= 30;
  return score;
}

/**
 * لایهٔ تصمیم‌گیری ترکیبی (ensemble) بین مسیر نمونه‌محور (k-NN با برچسب‌های
 * تأییدشدهٔ سایت) و مدل CNN:
 *  - هر دو مسیر روی یک تصویر اجرا می‌شوند و خروجی‌شان کاراکتر به کاراکتر
 *    مقایسه می‌شود — نه انتخاب کورکورانهٔ یک مسیر.
 *  - توافق: اطمینان = بیشینهٔ دو (پاداش تأیید متقابل).
 *  - اختلاف: برنده اطمینان بالاتر است، اما فقط اگر برتری‌اش قابل‌توجه باشد
 *    (حاشیهٔ امن) و در هر حال اطمینان ۱۰٪ کاهش می‌یابد تا پاسخ‌های مشکوک
 *    کمتر «مطمئن» جلوه کنند.
 * خروجی: { text, confidence, minConf, chars, variant } یا null.
 */
function ensembleProtoCnn(pr, cmr) {
  if (pr && cmr && Array.isArray(pr.chars) && Array.isArray(cmr.chars) && pr.chars.length === cmr.chars.length) {
    const chars = [];
    let text = '', confSum = 0, minConf = 1, agree = 0;
    const MARGIN = 10; // حداقل برتری اطمینان (درصد) برای چرخاندن رأی به سمت CNN
    for (let i = 0; i < pr.chars.length; i++) {
      const p = pr.chars[i], c = cmr.chars[i]; // conf هر دو درصدی است
      if (p.digit === c.digit) {
        agree++;
        const conf = Math.min(0.99, Math.max(p.conf, c.conf) / 100);
        text += p.digit;
        confSum += conf;
        if (conf < minConf) minConf = conf;
        chars.push({ digit: p.digit, conf: Math.round(conf * 100), source: 'proto+cnn' });
      } else {
        let win = p, src = 'prototype';
        if (c.conf > p.conf + MARGIN) { win = c; src = 'char-cnn'; }
        const loser = win === p ? c : p;
        const conf = (win.conf / 100) * 0.9; // جریمهٔ اختلاف نظر
        text += win.digit;
        confSum += conf;
        if (conf < minConf) minConf = conf;
        chars.push({ digit: win.digit, conf: Math.round(conf * 100), source: src + '(disagree:' + loser.digit + ')' });
      }
    }
    const confidence = Math.round((confSum / pr.chars.length) * 100);
    return {
      text, confidence, minConf, chars,
      variant: agree === pr.chars.length ? 'prototype+cnn' : 'ensemble',
    };
  }
  if (pr) return { ...pr, variant: 'prototype' };
  if (cmr) return { ...cmr, variant: 'char-model' };
  return null;
}
...
async function solveCaptcha(buffer, options = {}) {
  ...
  // ⚙️ مسیر موتور بازطراحی‌شده (نسخه ۳): قطبیت چند هیوریستیک + قطعه‌بندی
  // چندعاملی + اطمینان کالیبره. «نامزد اول» است و هر خطایی داشته باشد بی‌صدا
  // به مسیرهای اثبات‌شدهٔ قبلی فال‌بک می‌شود (جریان رزرو هرگز نمی‌شکند).
  let engineCandidate = null;
  if (CAPTCHA_CFG.use_ocr_engine !== false && !fd) {
    try {
      const { solveImage } = require('./ocr/engine');
      const ocrCfg = require('./ocr/config').loadOcrConfig();
      const expected = (MIN_LENGTH === MAX_LENGTH) ? MIN_LENGTH : null;
      const er = await solveImage(buffer, {
        config: ocrCfg,
        expectedLength: expected,
        minLength: MIN_LENGTH,
        maxLength: MAX_LENGTH,
        threshold: ocrCfg.confidence.seqThreshold,
        protos: USE_PROTOTYPES ? loadPrototypesCached() : [],
        log: true,
      });
      if (er && er.text) {
        const lenOk = er.text.length >= MIN_LENGTH && er.text.length <= MAX_LENGTH;
        if (er.ok && lenOk) {
          return {
            ok: er.confidence >= minConf,
            text: er.text,
            ...
            lengthOk: lenOk,
            confident: er.confidence >= minConf,
            passesRun: 0,
            chars: (er.chars || []).map((c) => ({ digit: c.ch, conf: Math.round((c.conf || 0) * 100), source: 'ocr-engine' })),
            seqConfidence: er.seqConfidence,
            meanCharConf: er.meanCharConf,
            minCharConf: er.minCharConf,
          };
        }
        if (lenOk) {
          engineCandidate = {
            text: er.text, confidence: Math.round((er.seqConfidence || 0) * 100),
            raw: null, variant: 'ocr-engine', whitelist: 'mixed',
          };
        }
      }
    } catch (e) { /* فال‌بک به مسیرهای قدیمی */ }
  }
  ...
  if (fd) {
    ...
    // مسیر الفبایی (واقعیت فعلی سایت): نمونه‌محور + CNN هر دو اجرا می‌شوند و
    // لایهٔ تصمیم‌گیری آن‌ها را ترکیب می‌کند. توجه: مسیر CNN به تنظیمهٔ
    // «نمونه‌محور» وابسته نیست — حتی با حافظهٔ خالی (استقرار تازه) کار می‌کند.
    try {
      const pr = USE_PROTOTYPES ? await solveWithPrototypes(buffer) : null;
      const cmr = await solveWithCharModel(buffer);
      const combined = ensembleProtoCnn(pr, cmr);
      if (combined) {
        const lenOk = combined.text.length >= MIN_LENGTH && combined.text.length <= MAX_LENGTH;
        if (lenOk && combined.minConf >= MODEL_MIN_CHAR_CONF) {
          return {
            ok: combined.confidence >= minConf,
            text: combined.text,
            ...
            minConfidence: minConf,
            lengthOk: lenOk,
            confident: combined.confidence >= minConf,
            passesRun: 0,
            chars: combined.chars,
          };
        }
        if (lenOk) modelCandidate = { text: combined.text, confidence: combined.confidence, raw: null, variant: combined.variant, whitelist: 'mixed' };
      }
    } catch (e) { /* مسیر تِسِرَکت ادامه می‌یابد */ }
  }

  // اگر مسیرهای مدل قدیمی نامزدی نساختند، نامزد موتور جدید وارد رقابت می‌شود
  if (!modelCandidate && engineCandidate) modelCandidate = engineCandidate;
...
module.exports = {
  ...
  advancedVariants, buildCleanVariant, scoreResult, ensembleProtoCnn,
