  ...
  try {
    ...
    if (fs.existsSync(cnnPath)) {
      ...
      if (json && json.classes && json.weights) {
        ...
        _charModel = { engine: 'cnn', model, classes: json.classes };
        return _charModel;
      }
    }
  ...
  try {
    ...
    if (fs.existsSync(modelPath)) {
      ...
      if (json && json.classes && json.layers) {
        ...
        _charModel = { engine: 'mlp', model: MLP.fromJSON(json), classes: json.classes };
      }
    }
...
async function solveWithCharModel(buffer) {
  ...
  return {
    text,
    confidence: Math.round((confSum / comps.length) * 100),
    minConf,
    chars,
    engine: cm.engine,
    variant: 'char-model',
...
function buildModelCandidateResult(candidate, forceDigits = FORCE_DIGITS) {
  if (!candidate || typeof candidate.text !== 'string') return null;
  const out = {
    text: candidate.text,
    confidence: candidate.confidence,
    raw: candidate.raw || null,
    variant: candidate.variant || 'custom-model',
    whitelist: candidate.whitelist || (forceDigits ? 'digits' : 'mixed'),
  };
  if (candidate.engine) out.engine = candidate.engine;
  if (Array.isArray(candidate.chars)) out.chars = candidate.chars;
  out.score = scoreResult(out, forceDigits);
  return out;
}
...
async function solveCaptcha(buffer, options = {}) {
  ...
  if (fd) {
    try {
      ...
      if (mr) {
        ...
        if (lenOk) modelCandidate = { ...mr, variant: 'custom-model', whitelist: 'digits', engine: mr.engine || 'mlp' }; // برای رقابت با نتایج تِسِرَکت نگه می‌داریم
      }
    ...
    try {
      ...
      if (pr) {
        ...
        if (lenOk) modelCandidate = { text: pr.text, confidence: pr.confidence, raw: null, variant: 'prototype', whitelist: 'mixed', engine: pr.engine || 'prototype', chars: pr.chars };
      }
      ...
      if (cmr) {
        ...
        if (lenOk && !modelCandidate) modelCandidate = { text: cmr.text, confidence: cmr.confidence, raw: null, variant: 'char-model', whitelist: 'mixed', engine: cmr.engine, chars: cmr.chars };
      }
    } catch (e) { /* مسیر تِسِرَکت ادامه می‌یابد */ }
  }
  ...
  try {
    ...
    const results = [];
    if (modelCandidate) {
      // نتیجه شاخهٔ داخلی به‌عنوان نامزد وارد رقابت می‌شود؛ provenance واقعی‌اش
      // باید برای benchmark/telemetry حفظ شود.
      const r = buildModelCandidateResult(modelCandidate, fd);
      if (r) results.push(r);
    }
...
module.exports = {
  ...
  advancedVariants, buildCleanVariant, scoreResult, buildModelCandidateResult,
