...
async function solveWithModel(buffer) {
  ...
  try {
    ...
    for (const run of runs) {
      ...
      chars.push({ digit: LABELS[label], conf: Math.round(probs[label] * 100) });
    }
    return {
      text,
...
async function solveCaptcha(buffer, options = {}) {
  ...
  // ۰) مسیر سریع: مدل اختصاصی. اگر با اطمینان بالا حل کرد، اصلاً سراغ
  // تِسِرَکت نمی‌رویم (سریع‌تر و دقیق‌تر برای کپچای این سایت).
