...
/**
 * تطبیق با نمونه‌های واقعی: رأی‌گیری وزن‌دار k-نزدیک (k≤3) بین نامزدهای داخل
 * آستانه. وزن هر رأی ۱/(فاصله+۰٫۰۵) است؛ در عمل نزدیک‌ترین نمونه غالب است و
 * رأی‌گیری فقط وقتی نتیجه را عوض می‌کند که چند نمونهٔ نزدیکِ هم‌کلاس، نویزِ
 * یک تطبیق لبه‌ای از کلاس دیگر را خنثی کنند. خروجی { digit, dist } که dist
 * کمترین فاصلهٔ کلاس برنده است (پایهٔ محاسبهٔ اطمینان ۱−dist، مثل قبل).
 */
function matchPrototype(vec, prototypes, maxDist = 0.3, k = 3) {
  const cand = [];
  for (const p of prototypes || []) {
    if (!p || !Array.isArray(p.v)) continue;
    const d = robustDist(vec, p.v);
    if (d <= maxDist) cand.push({ digit: p.digit, dist: d });
  }
  if (!cand.length) return null;
  cand.sort((a, b) => a.dist - b.dist);
  const top = cand.slice(0, k);
  const votes = new Map();
  for (const c of top) votes.set(c.digit, (votes.get(c.digit) || 0) + 1 / (c.dist + 0.05));
  let winner = top[0].digit, best = -1;
  for (const [digit, w] of votes) {
    if (w > best) { best = w; winner = digit; }
  }
  let bestDist = Infinity;
  for (const c of top) if (c.digit === winner && c.dist < bestDist) bestDist = c.dist;
  return { digit: winner, dist: bestDist };
}
...
/**
 * بارگذاری کامل دادهٔ آموزشی برای بازآموزی مدل (نسخه ناهمگون/آسینک).
 *
 * تفاوت با loadPrototypes (که فقط بردارهای ازقبل‌ذخیره‌شده را می‌خواند):
 * نمونه‌هایی که در حلقه یادگیری ذخیره شده‌اند ولی بردار کاراکتر ندارند
 * (مثلاً استخراج در لحظه ذخیره‌سازی به‌خاطر نویز/چسبیدگی ناموفق بوده) اینجا
 * دوباره و با همان خط لوله استنتاج از روی تصویر استخراج می‌شوند. به این ترتیب
 * هیچ کپچای تأییدشده‌ای که قابل استخراج باشد از آموزش CNN جا نمی‌ماند.
 *
 * خروجی: { protos: [{ digit, v }], stats: { samples, withVectors, extractedNow, failed } }
 */
async function loadTrainingPrototypes() {
  let dbj = {};
  try { dbj = JSON.parse(fs.readFileSync(dbFilePath(), 'utf8')); } catch (e) { dbj = {}; }
  const samples = dbj.captcha_samples || [];
  const out = [];
  const stats = { samples: samples.length, withVectors: 0, extractedNow: 0, failed: 0 };
  for (const s of samples) {
    // ۱) بردارهای ذخیره‌شده (مسیر اصلی)
    if (Array.isArray(s.char_vectors) && s.char_vectors.length) {
      let used = 0;
      for (const cv of s.char_vectors) {
        if (cv && /^[A-Za-z0-9]$/.test(cv.digit) && Array.isArray(cv.v) && cv.v.length === VEC_SIZE) {
          out.push({ digit: cv.digit, v: cv.v });
      ...
      if (used) { stats.withVectors++; continue; }
    }
    // ۲) فال‌بک: استخراج از تصویر نمونه (همان خط لوله استنتاج)
    const b64 = String(s.image || '').split(',')[1];
    const text = String(s.text || '');
    if (!b64 || !/^[A-Za-z0-9]{3,8}$/.test(text)) { stats.failed++; continue; }
    try {
      const vecs = await extractCharVectors(Buffer.from(b64, 'base64'), text);
      if (vecs && vecs.length === text.length) {
        for (const cv of vecs) out.push(cv);
        stats.extractedNow++;
      } else {
        stats.failed++;
      }
...
module.exports = {
  ...
  extractCharVectors, robustDist, matchPrototype, loadPrototypes,
  loadTrainingPrototypes, dbFilePath,
