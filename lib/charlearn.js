 ...
 */
function segmentColoredChars(jimg) {
...
function extractComponents(jimg) {
  return segmentColoredChars(jimg) || grayscaleChars(jimg);
}
...
async function extractCharVectors(buffer, label) {
  ...
  const comps = extractComponents(jimg);
...
module.exports = {
  ...
  extractCharVectors, robustDist, matchPrototype, loadPrototypes, dbFilePath, VEC_SIZE,
};
