 ...
 * - کلید امضای نشست از lib/secrets.js می‌آید: کلیدی *جدا* از کلید لایسنس
 *   (BILITFAST_SESSION_KEY یا مشتق‌شده از BILITFAST_MASTER_KEY). در محیط
 *   تولید، نبودِ کلید باعث توقف سرویس می‌شود (fail-closed) تا هرگز با کلید
 *   پیش‌فرضِ قابل حدس، توکن نشست امضا نشود.
 */
...
function secretKey() {
  // کلید مخصوص نشست؛ در تولید اگر تنظیم نشده باشد، خطا می‌دهد (fail-closed).
...
function registerUser(username, password) {
  ...
  const user = db.insert('users', {
    ...
    payment_notify: { enabled: false, bale: false, eitaa: false, bale_chat_id: '', eitaa_chat_id: '', template: '' },
...
function publicUser(user) {
  ...
  return {
    ...
    payment_notify: {
      enabled: !!(user.payment_notify && user.payment_notify.enabled),
      telegram: !!(user.payment_notify && user.payment_notify.telegram),
      bale: !!(user.payment_notify && user.payment_notify.bale),
      eitaa: !!(user.payment_notify && user.payment_notify.eitaa),
      bale_chat_id: (user.payment_notify && user.payment_notify.bale_chat_id) || '',
      eitaa_chat_id: (user.payment_notify && user.payment_notify.eitaa_chat_id) || '',
      template: (user.payment_notify && user.payment_notify.template) || '',
    },
