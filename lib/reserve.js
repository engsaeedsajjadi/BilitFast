...
// لایه شبکه بهینه (keep-alive + فشرده‌سازی) — برقراری اتصال TLS فقط یک بار
const { safirFetch } = require('./http');

/**
 * محافظ SSRF: فقط data-URI، مسیرهای نسبی و آدرس‌هایی که میزبانشان با
 * دامنه پیکربندی‌شده صفیر ریل یکی است مجازند. آدرس‌های مطلق به دامنه‌های
 * دیگر (نشانی داخلی مثل 169.254.169.254، localhost و...) رد می‌شوند.
 */
function isSafirUrl(u) {
  ...
  // نسبی (با / شروع شود یا بدون پروتکل) — ربات fetch نسبی را به base می‌چسباند؟
  // fetch مقصد نسبی را با آبجکت URL سرور خودش تفسیر می‌کند؛ بنابراین برای
  // اطمینان، نسبی‌ها را فقط با پیشوند / می‌پذیریم.
  return u.startsWith('/');
}
function assertSafirUrl(u) {
  if (!isSafirUrl(u)) {
    throw new Error('نشانی تصویر/درخواست خارج از دامنه مجاز است و رد شد.');
  }
}
...
async function fetchFollowing(url, opts = {}) {
  ...
  const isTransient = (msg) => /fetch failed|ENOTFOUND|ECONNREFUSED|ECONNRESET|socket hang up|TLS|SSL|ETIMEDOUT|EPIPE|EOF/i.test(msg);

  // تلاش مجدد خودکار روی خطاهای گذرای شبکه (مثلاً ریست/تایم‌اوت موقت در
  // پستِ captchaAjax.php). درخواست GET اول ممکن است موفق شده باشد ولی پستِ
  // بعدی به‌خاطر یک ریست TLS بیفتد؛ مرورگر در این حالت خودکار دوباره تلاش می‌کند.
  let lastErr;
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      // خطای «مقصد مجاز نیست» (SSRF) یا تایم‌اوتِ اعلامی، قابل تکرار نیستند
      const declared = msg.startsWith('اتصال به صفیر ریل بیش از') || msg.includes('خارج از دامنه مجاز');
      if (!isTransient(msg) || declared || attempt === retries) break;
      await sleep(400 * (attempt + 1)); // ۴۰۰ms، سپس ۸۰۰ms
    }
  }
  throw lastErr;
  ...
  for (let i = 0; i <= maxRedirects; i++) {
    ...
    if ([301, 302, 303, 307, 308].includes(status)) {
      ...
      if (!isSafirUrl(next)) break; // تغییرمسیر به دامنه بیرونی را دنبال نمی‌کنیم (SSRF)
      current = next;
...
function analyzeHtml(htmlText, baseUrl) {
  ...
  // --- آدرس ریدایرکت جاوااسکریپت (مثل document.location="https://...shaparak.../?Token=...") ---
  // صفحه پرداخت صفیر ریل به درگاه بانک (شاپرک) با یک ریدایرکت JS هدایت می‌شود، نه یک
  // فرم action. پس آدرس مطلق ریدایرکت را از خود HTML استخراج می‌کنیم تا درگاه درست
  // باز شود (نه خود صفحه VerifyTck.php که بدون کوکی نشست دوباره قابل بازپخش نیست).
  let redirectUrl = null;
  const redirectRe = /(?:document|window)\.location(?:\.href)?\s*=\s*["']([^"']+)["']/g;
  let rm;
  while ((rm = redirectRe.exec(htmlText)) !== null) {
    const u = String(rm[1] || '').trim();
    // فقط آدرس‌های مطلق http(s) (درگاه بانک)؛ ریدایرکت‌های نسبی مثل index.php رد می‌شوند
    if (/^https?:\/\//i.test(u)) { redirectUrl = u; break; }
  }
...
function mergeCaptchaAjax(analysis, parsed) {
  ...
  const ajaxResp = b64 ? (parsed.captchaId + '@' + b64) : (merged.hiddenFields.ajaxResponse || '');
  merged.hiddenFields = {
    ...(merged.hiddenFields || {}),
    captchaId: parsed.captchaId,
    ajaxResponse: ajaxResp,
  };
  // formFieldValues هم باید به‌روز شود، چون بدنه ارسال از آن ساخته می‌شود.
  // در غیر این صورت captchaId/ajaxResponse خالیِ صفحه اولیه (که قبل از اجرای JS
  // خالی هستند) جای مقدار تازه را می‌گیرند و سرور کپچا را نمی‌تواند اعتبارسنجی کند.
  merged.formFieldValues = {
    ...(merged.formFieldValues || {}),
    captchaId: parsed.captchaId,
    ajaxResponse: ajaxResp,
  };
...
/* =====================================================================
 * توکن وضعیت (stateless) — رمزنگاری‌شده (AES-256-GCM + انقضا)
 * =====================================================================
 * وضعیت رزرو (شامل کوکی سشن و داده مسافران) باید بین درخواست‌ها برگردد اما
 * نباید قابل خواندن/دستکاری باشد. جزئیات در lib/token.js.
 * نکته: توکن‌های قدیمی (base64 ساده) دیگر باز نمی‌شوند؛ کاربر کافی است رزرو
 * را از نو شروع کند (وضعیت رزرو کلاً موقتی و چنددقیقه‌ای است).
 */
  ...
  if (needsAjaxCaptcha(analysis)) {
    ...
    try {
      ...
      if (parsed) {
        ...
        // کوکی/نشست احتمالی captchaAjax.php را هم در state نهایی نگه می‌داریم،
        // تا ارسال کپچا با همان نشستی انجام شود که کپچا به آن گره خورده است.
        result.jar = ajaxResult.jar;
...
async function submitReservation({ stateToken, captcha, passengers, phone }) {
  ...
  // ولیدیشن سمت سرور (اعتماد به کلاینت کافی نیست): کد ملی مسافران ایرانی
  // باید ۱۰ رقم و معتبر باشد. اتباع خارجی (کد گذرنامه) از این قاعده مستثنا هستند.
  for (const p of Array.isArray(passengers) ? passengers : []) {
    const qt = p && p.quota_type;
    const nc = p && String(p.national_code || '').trim();
    if (qt === 'اتباع') continue;
    if (nc && !isValidIranianNationalCode(nc)) {
      return { ok: false, step: 'validation_error', error: 'کد ملی یک مسافر نامعتبر است: ' + nc.slice(0, 3) + '*** (لطفاً آن را بررسی کنید).' };
    }
  }
  ...
  // ترتیب مهم است: hiddenFields (حاوی captchaId/ajaxResponse تازه از captchaAjax.php)
  // باید روی formFieldValues (که از DOM صفحه با captchaId/ajaxResponse خالی ساخته شده)
  // بنویسد تا کپچا درست اعتبارسنجی شود. formFieldValues فیلدهای visible/radio را نگه
  // می‌دارد و hiddenFields فقط مقادیر مخفی تازه را روی آن می‌نویسد.
  const body = {
  ...
  // --- لاگ تشخیصی غیرحساس: چه چیزهایی واقعاً ارسال می‌شود (بدون چاپ مقادیر) ---
  // این لاگ مشخص می‌کند که آیا کپچا/captchaId/ajaxResponse/نشست واقعاً در بدنه است
  // یا نه — بدون افشای خود کپچا، captchaId یا ajaxResponse.
  // نکته: نام کلیدها عمداً کلمه «captcha» را ندارند، چون maskValue هر کلیدی که
  // شامل «captcha/securitycode/kaptcha» باشد را به «***» تبدیل می‌کند.
  logEvent(workflow, 'SUBMIT_DEBUG', {
    codeProvided: isCaptchaSubmit,
    codeInputName: state.captchaInputName || null,
    codeIdPresent: !!(body.captchaId && String(body.captchaId).length > 0),
    ajaxResponsePresent: !!(body.ajaxResponse && String(body.ajaxResponse).length > 0),
    sessionPresent: !!(state.jar && state.jar.some((c) => /PHPSESSID/i.test(String(c)))),
    formFieldCount: Object.keys(body).length,
    hiddenFieldCount: Object.keys(state.hiddenFields || {}).length,
    srvcPresent: !!(body.srvc && String(body.srvc).length > 0),
    target: action,
...
function extractPaymentUrl(analysis, result) {
  // ۱) ریدایرکت جاوااسکریپت به درگاه (مثل document.location="https://pec.shaparak.ir/NewIPG/?Token=...")
  //    این آدرس Token درگاه را دارد و مستقیماً قابل باز شدن است.
  if (analysis.redirectUrl) return analysis.redirectUrl;
  // ۲) اگر صفحه حاوی فرم پرداخت با action باشد، آن را برگردان
  const action = analysis.mainFormAction;
  if (action) return absoluteUrl(action, result.finalUrl);
  // ۳) در غیر این صورت، خود صفحه
...
async function refreshCaptcha({ stateToken }) {
  ...
  // به‌روزرسانی state: captchaId جدید + ajaxResponse جدید + کوکی‌جار جدید.
  // هر دو hiddenFields و formFieldValues باید به‌روز شوند، وگرنه ارسال نهایی
  // از captchaId/ajaxResponse قدیمی (یا خالی) استفاده می‌کند.
  const b64 = String(parsed.dataUri || '').split(',')[1] || '';
  const ajaxResp = b64 ? (parsed.captchaId + '@' + b64) : '';
  const newState = {
    ...state,
    jar: ajaxResult.jar,
    hiddenFields: {
...
async function fetchCaptchaImage(captchaImageUrl, stateToken) {
  ...
  if (!isSafirUrl(captchaImageUrl)) {
    return { ok: false, error: 'نشانی تصویر کپچا خارج از دامنه مجاز است.' };
  }
  const state = decodeState(stateToken);
  const jar = state ? state.jar : [];
  try {
    const resp = await safirFetch(captchaImageUrl, {
...
async function fetchCaptchaBuffer(captchaImageUrl, stateToken) {
  const { assertSafeImage } = require('./image-guard');
  // کپچای data-URI (جاسازی‌شده) → بدون fetch
  const d = dataUriToBuffer(captchaImageUrl);
  if (d) { assertSafeImage(d); return d; }
  if (!isSafirUrl(captchaImageUrl)) {
    throw new Error('نشانی تصویر کپچا خارج از دامنه مجاز است.');
  }
  ...
  const buf = Buffer.from(await resp.arrayBuffer());
  // فقط قالب‌های تصویری واقعی به موتور پردازش تصویر می‌رسند؛ ورودی خراب/غیرتصویری
  // (مسیر آسیب‌پذیر ASF در file-type) همین‌جا رد می‌شود.
  assertSafeImage(buf);
  return buf;
}
...
async function solveCaptchaImage({ captchaImageUrl, stateToken }) {
  ...
  try {
    ...
    // حلقه یادگیری: ثبت خودکارِ تصویر کپچا + پیش‌بینی مدل. بعد از ارسال رزرو،
    // نتیجه (پذیرش/رد توسط صفیر ریل) روی همین رکورد ثبت می‌شود تا نمونه‌های
    // تأییدشده به داده آموزشی تبدیل شوند. «بهترین تلاش» — هرگز نباید جریان
    // رزرو را مختل کند.
    try {
      if (result && result.text) {
        const { recordCapture } = require('./captures');
        const isPng = buf.length > 3 && buf[0] === 0x89 && buf[1] === 0x50;
        const dataUri = 'data:' + (isPng ? 'image/png' : 'image/jpeg') + ';base64,' + buf.toString('base64');
        const captureId = recordCapture({
          image: dataUri,
          text: result.text,
    ...
    } catch (e) { /* نادیده بگیر — جریان رزرو مهم‌تر است */ }
    return result;
  } catch (e) {
    return { ok: false, error: (e && e.message) ? e.message : String(e) };
  }
...
module.exports = {
  ...
  absoluteUrl,
  fetchFollowing,
  __isTransientNetworkError: (msg) => /fetch failed|ENOTFOUND|ECONNREFUSED|ECONNRESET|socket hang up|TLS|SSL|ETIMEDOUT|EPIPE|EOF/i.test(String(msg || '')),
  analyzeHtml,
  isSafirUrl,
