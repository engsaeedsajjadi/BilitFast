...
async function submitReservation({ stateToken, captcha, passengers, phone }) {
  ...
  // ساختن بدنه ارسال: پارامترهای رزرو + فیلدهای مخفی + مقادیر select + کپچا + مسافر
  // (در سایت، mainFrm صفحه رزرو همه پارامترهای رزرو را به‌صورت hidden دارد؛
  // برای اطمینان، reserveData را هم به‌عنوان پایه می‌گذاریم.)
  const body = { ...(state.reserveData || {}), ...(state.hiddenFields || {}), ...(state.selectValues || {}) };
  ...
  // هدف ارسال: در سایت، chkForm() action فرم را VerifyTck.php قرار داده و
  // ارسال می‌کند (تأیید رزرو → صفحه پرداخت). اگر action فرم به‌دلیل رندر JS
  // خالی بود، مستقیماً به VerifyTck.php ارسال می‌کنیم.
  let action;
