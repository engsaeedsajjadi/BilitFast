...
function analyzeHtml(htmlText, baseUrl) {
  ...
  // وقتی سرور رزرو را رد می‌کند، معمولاً یک alert مثل
  //   alert('101-متاسفانه ارائه سرویس رفت درخواست شده امکان‌پذیر نمی‌باشد')
  // در صفحه می‌گذارد. این پیام‌ها برای نمایش خطای واقعی به کاربر استخراج می‌شوند.
  const serverMessages = [];
  const alertRe = /alert\s*\(\s*(['"])(.*?)\1\s*\)/g;
  let am;
  while ((am = alertRe.exec(htmlText)) !== null) {
      ...
      if (empty) {
        if (serverMsg) {
          message = 'سامانه این رزرو را رد کرد: «' + serverMsg + '». ' +
            'این معمولاً یعنی قطار دیگر قابل رزرو نیست (ظرفیت تکمیل/سرویس لغو) یا کوکی نشست منقضی شده است. ' +
            'دوباره جستجو کنید و قطار دیگری را امتحان کنید.';
        } else {
