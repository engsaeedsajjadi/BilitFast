...
function analyzeHtml(htmlText, baseUrl) {
  ...
  // تشخیص گسترده‌تر کپچا: تصویر/اسکریپت ممکن است با AJAX (فیلد ajaxResponse)
  // لود شود یا در src/action/iframe با الگوی متفاوتی بیاید.
  if (!captchaImgUrl) {
    const m = htmlText.match(/(?:src|href|data-src|action)\s*=\s*["']([^"']*(?:kcaptcha|captcha|kaptcha|securitycode)[^"']*)["']/i);
    if (m) captchaImgUrl = m[1];
  }
  ...
  $('form').each((_fi, fel) => {
    ...
    $(fel).find('input, select, textarea, button').each((_i, el) => {
      ...
      fields.push({ tag, name, id, effName, type, value, placeholder, onclick, onchange });
  ...
  // --- پیدا کردن فیلد کپچا (input متنی که name/id/placeholder آن به کپچا می‌خورد) ---
