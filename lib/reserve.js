...
const { makeCaptchaLearnToken } = require('./license');

const RC = config.reservation || {};
const SAFIR_ORIGIN = new URL(config.base_url).origin;
...
function resolveSafirUrl(url, base) {
  return absoluteUrl(url, base || config.base_url);
}

function isAllowedSafirUrl(url, base) {
  const u = resolveSafirUrl(url, base);
  if (!u || /^data:/i.test(u)) return false;
  try {
    const parsed = new URL(u);
    return /^https?:$/i.test(parsed.protocol) && parsed.origin === SAFIR_ORIGIN;
  } catch (e) {
    return false;
  }
}

function cookieHeaderForUrl(jar, url) {
  return isAllowedSafirUrl(url) ? cookieHeader(jar) : '';
}

function hashCaptchaDataUri(dataUri) {
  const m = /^data:[^;]*;base64,(.*)$/i.exec(String(dataUri || ''));
  if (!m) return '';
  try {
    return crypto.createHash('sha256').update(Buffer.from(m[1], 'base64')).digest('hex');
  } catch (e) {
    return '';
  }
}
...
async function fetchFollowing(url, opts = {}) {
  ...
  for (let i = 0; i <= maxRedirects; i++) {
    const h = { ...headers };
    const scopedCookie = currentJar.length ? cookieHeaderForUrl(currentJar, current) : '';
    if (scopedCookie) h['Cookie'] = scopedCookie;
...
async function submitReservation({ stateToken, captcha, passengers, phone }) {
  ...
  if (isCaptchaSubmit && stepResp.step !== 'captcha' && state.captchaImageHash) {
    stepResp.learnToken = makeCaptchaLearnToken({
      workflow_id: workflow.workflow_id,
      captcha_text: String(captcha || '').trim(),
      image_hash: state.captchaImageHash,
...
function buildStepResponse(analysis, state, result, passengers, workflow) {
  ...
  const captchaImageHash = analysis.captchaDataUri
    ? hashCaptchaDataUri(analysis.captchaDataUri)
    : (state.captchaImageHash || '');
...
async function refreshCaptcha({ stateToken }) {
  ...
  const imageHash = hashCaptchaDataUri(parsed.dataUri);
  return {
    ok: true,
    captchaId: parsed.captchaId,
    ...
    imageHash,
    stateToken: encodeState({ ...newState, captchaImageHash: imageHash || state.captchaImageHash || '' }),
  };
}
...
async function fetchCaptchaImage(captchaImageUrl, stateToken) {
  const state = decodeState(stateToken);
  if (!stateToken || !state) {
    return { ok: false, error: 'وضعیت رزرو نامعتبر یا منقضی شده است.' };
  }
  // اگر کپچا از قبل data-URI باشد (توی HTML جاسازی شده)، مستقیم همان را برگردان
  if (/^data:image/i.test(captchaImageUrl || '')) {
    const imageHash = hashCaptchaDataUri(captchaImageUrl);
    return {
      ok: true,
      dataUri: captchaImageUrl,
      imageHash,
      stateToken: encodeState({ ...state, captchaImageHash: imageHash || state.captchaImageHash || '' }),
    };
  }
  const jar = state.jar || [];
  const resolvedUrl = resolveSafirUrl(captchaImageUrl, state.fromUrl || config.base_url);
  if (!isAllowedSafirUrl(resolvedUrl)) {
    return { ok: false, error: 'آدرس تصویر کپچا معتبر نیست.' };
  }
  try {
    const resp = await fetch(resolvedUrl, {
      headers: { ...buildHeaders(), Cookie: cookieHeaderForUrl(jar, resolvedUrl) },
    });
    ...
    return {
      ...
      dataUri,
      imageHash,
      stateToken: encodeState({ ...state, captchaImageHash: imageHash }),
    };
  } catch (e) {
...
async function fetchCaptchaBuffer(captchaImageUrl, stateToken) {
  const state = decodeState(stateToken);
  if (!stateToken || !state) throw new Error('وضعیت رزرو نامعتبر یا منقضی شده است.');
  ...
  const jar = state.jar || [];
  const resolvedUrl = resolveSafirUrl(captchaImageUrl, state.fromUrl || config.base_url);
  if (!isAllowedSafirUrl(resolvedUrl)) throw new Error('آدرس تصویر کپچا معتبر نیست.');
  const resp = await fetch(resolvedUrl, {
    headers: { ...buildHeaders(), Cookie: cookieHeaderForUrl(jar, resolvedUrl) },
  });
