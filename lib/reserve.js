...
function analyzeHtml(htmlText, baseUrl) {
  ...
  $('form').each((_fi, fel) => {
    ...
    $(fel).find('input, select, textarea, button').each((_i, el) => {
      ...
      fields.push({ tag, name, id, effName, type, value, placeholder, onclick, onchange, selectedValue });
  ...
  // --- فیلدهای مسافر (ورودی‌های قابل‌پرکردن، بدون کپچا/غذا/رادیو) ---
  ...
  // لود صفحه با AJAX تزریق می‌شوند (captcha.innerHTML). برای فهمیدن جریان واقعی،
  // محتوای اسکریپت و onclick دکمه‌ها ثبت می‌شود.
  const scripts = [];
  $('script').each((_i, el) => {
    const txt = ($(el).html() || '').replace(/\s+/g, ' ').trim();
    if (txt) scripts.push(maskHtmlForDiagnostics(txt).slice(0, 1500));
  });
  const handlers = [];
  if (mainForm) {
    for (const f of mainForm.fields) {
...
/** ادغام نتیجه AJAX (کپچا/فیلد مسافر) در تحلیل صفحه اصلی. */
function mergeAjaxAnalysis(analysis, ajax) {
  if (!ajax) return analysis;
  const merged = { ...analysis };
  if (!merged.captchaDataUri && !merged.captchaImgUrl) {
    merged.captchaDataUri = ajax.captchaDataUri;
    merged.captchaImgUrl = ajax.captchaImgUrl;
    merged.captchaInputName = ajax.captchaInputName;
  }
  // فیلدهای مسافر را ادغام کن (phone از پوسته + pid0..ln0 از پاسخ AJAX)
  const mergedPass = [...(merged.passengerFields || [])];
  const seenNames = new Set(mergedPass.map((f) => f.effName));
  for (const f of (ajax.passengerFields || [])) {
    if (!seenNames.has(f.effName)) {
      mergedPass.push(f);
      seenNames.add(f.effName);
    }
  }
  merged.passengerFields = mergedPass;
  if (ajax.hiddenFields) {
    merged.hiddenFields = { ...(merged.hiddenFields || {}), ...ajax.hiddenFields };
  }
  if (ajax.selectValues) {
    merged.selectValues = { ...(merged.selectValues || {}), ...ajax.selectValues };
  }
  if (ajax.mainFormAction) {
    merged.mainFormAction = ajax.mainFormAction;
    merged.mainFormMethod = ajax.mainFormMethod;
  }
  merged.ajax = {
    finalUrl: ajax.diag && ajax.diag.finalUrl,
    formCount: ajax.diag && ajax.diag.formCount,
    inputCount: ajax.diag && ajax.diag.inputCount,
    fieldNames: ajax.diag && ajax.diag.forms[0] && ajax.diag.forms[0].fieldNames,
    captchaDataUri: ajax.captchaDataUri ? '[data-uri]' : null,
    preview: ajax.diag && ajax.diag.rawHtml,
  };
  // بازطبقه‌بندی با داده‌های ادغام‌شده
  merged.classification = classifyForm({
    ...
    captchaImgUrl: merged.captchaImgUrl || merged.captchaDataUri,
...
/* =====================================================================
 * شروع رزرو — مرحله ۱ (GET به TresV.php)
 * ===================================================================== */
...
async function startReservation({ fields, passengers, train, cookies }) {
  ...
  // نکته: سامانه صفیر ریل پارامترهای رزرو را مانند جستجو (searchWagn.php) از
  // query-string می‌خواند نه از بدنه POST. ارسال POST به TresV.php باعث
  // می‌شود پارامترها دریافت نشوند و صفحه خالی (formCount=0) برگردد.
  // بنابراین درخواست شروع رزرو با GET و پارامترها در URL ارسال می‌شود.
  const action = config.base_url + config.reserve_url + '?' + toUrlEncoded(reserveData);
  let result;
  try {
  ...
  if (needsAjaxCaptcha(analysis)) {
    ...
    const ajaxBody = { ...analysis.hiddenFields, ...reserveData };
    try {
      const ajaxResult = await fetchFollowing(ajaxUrl, {
        method: 'POST',
        headers: buildHeaders(),
        body: toUrlEncoded(ajaxBody),
        jar: result.jar,
      });
      captchaAjaxMeta = {
        ...
        preview: maskHtmlForDiagnostics(ajaxResult.html).slice(0, 2000),
      };
      const ajaxAnalysis = analyzeHtml(ajaxResult.html, ajaxResult.finalUrl);
      analysis = mergeAjaxAnalysis(analysis, ajaxAnalysis);
      // لاگ نتیجه ادغام AJAX (کپچا/فیلدهای مسافر)
      logEvent(workflow, 'RESERVATION_AJAX_READY', {
        finalUrl: ajaxResult.finalUrl,
        httpStatus: captchaAjaxMeta.httpStatus,
        classification: (analysis.classification && analysis.classification.type) || 'unknown',
        ajaxFormCount: ajaxAnalysis.diag.forms.length,
        ajaxInputCount: ajaxAnalysis.diag.inputs.length,
        ajaxFieldNames: (ajaxAnalysis.diag.forms[0] && ajaxAnalysis.diag.forms[0].fieldNames) || [],
        captchaDataUri: analysis.captchaDataUri ? '[data-uri]' : null,
        captchaInputName: analysis.captchaInputName,
        mergedPassengerFields: (analysis.passengerFields || []).map((f) => f.effName),
      });
    } catch (e) {
...
async function submitReservation({ stateToken, captcha, passengers, phone }) {
  ...
  // ساختن بدنه ارسال: فیلدهای مخفی + مقادیر select + کپچا + اطلاعات مسافر
  const body = { ...(state.hiddenFields || {}), ...(state.selectValues || {}) };
  ...
  // هدف ارسال: اگر فرم action خالی بود (فرم JS-رندر)، مستقیماً به TresV.php
  // (بدون query-string) ارسال می‌کنیم.
  let action;
  if (state.mainFormAction) {
    action = absoluteUrl(state.mainFormAction, state.fromUrl);
  } else {
    action = config.base_url + config.reserve_url;
  }
  ...
  switch (cls.type) {
    ...
    case 'captcha':
      ...
      extra.captchaImageUrl = analysis.captchaDataUri
        ...
        : absoluteUrl(result.finalUrl, analysis.captchaImgUrl);
