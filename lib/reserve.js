...
function safeFieldValue(f) {
  ...
  if (/token|session|csrf|auth|sign|pass|secret/i.test(f.name + ' ' + v)) return undefined;
...
function analyzeHtml(htmlText, baseUrl) {
  ...
  $('form').each((_fi, fel) => {
    ...
    $(fel).find('input, select, textarea, button').each((_i, el) => {
      ...
      if (tag !== 'button') allInputs.push({ name, type });
    });
  ...
  // --- پیدا کردن فیلد کپچا (input متنی که نام/placeholder آن به کپچا می‌خورد) ---
  let captchaInputName = null;
  if (mainForm) {
    for (const f of mainForm.fields) {
      if (f.type !== 'hidden' && f.tag !== 'select' && f.tag !== 'textarea' && f.tag !== 'button') {
  ...
  // --- فیلدهای مسافر (ورودی‌های قابل‌پرکردن، بدون کپچا) ---
  ...
  if (mainForm) {
    for (const f of mainForm.fields) {
      if (f.type === 'hidden' && f.name) {
        hiddenFields[f.name] = f.value;
        hiddenFieldNames.push(f.name);
      }
    }
  }
  ...
  const diagForms = forms.map((f) => ({
    action: f.action,
    method: f.method,
    fieldNames: f.fields.map((x) => x.name),
    inputs: f.fields
      .filter((x) => x.tag !== 'button')
      .map((x) => {
        const v = safeFieldValue(x);
        return v === undefined ? { name: x.name, type: x.type } : { name: x.name, type: x.type, value: v };
      }),
    hiddenFieldNames: f.fields.filter((x) => x.type === 'hidden').map((x) => x.name),
    selectNames: f.fields.filter((x) => x.tag === 'select').map((x) => x.name),
    buttonNames: f.fields.filter((x) => x.tag === 'button').map((x) => x.name),
  }));
  ...
  const diag = {
    ...
    images,
    hiddenFieldNames,
    selectNames: mainForm ? mainForm.fields.filter((x) => x.tag === 'select').map((x) => x.name) : [],
    buttonNames: mainForm ? mainForm.fields.filter((x) => x.tag === 'button').map((x) => x.name) : [],
...
function classifyPassenger(passengerFields) {
  ...
  for (const f of passengerFields) {
    const { base } = splitFieldName(f.name);
    const key = findFieldKey(base, f.name, f.placeholder, hints, keyByHint);
    if (key) {
      matched.add(key);
      evidence.push(f.name + '→' + key);
    }
  }
...
async function submitReservation({ stateToken, captcha, passengers, phone }) {
  ...
  let action = absoluteUrl(state.mainFormAction || '', state.fromUrl || config.base_url + config.reserve_url);
...
function buildStepResponse(analysis, state, result, passengers, workflow) {
  ...
  return {
    ...
    flags: {
      ...
      hasCaptchaImage: !!(analysis.captchaImgUrl),
    },
    finalUrl: result.finalUrl,
    httpStatus: result.resp ? result.resp.status : 0,
