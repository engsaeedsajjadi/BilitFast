...
function analyzeHtml(htmlText, baseUrl) {
  ...
  // واقعی نیستند و نباید استخراج شوند؛ فقط alertهایی که بلافاصله با ریدایرکت
  // (document/window.location) دنبال می‌شوند، خطای قطعی سرور هستند.
  ...
  // fallback: اگر هیچ alert ریدایرکت‌دار نبود، همه alertها را جمع کن (برای تشخیص)
...
async function submitReservation({ stateToken, captcha, passengers, phone }) {
  ...
  // ساختن بدنه ارسال: فقط فیلدهای مخفی صفحه (وضعیت سرور) + selectها + کپچا + مسافر.
  // نکته مهم: پارامترهای جستجو (fromd/tod/departureTrain/returnTrain/groupWay/tmpDate)
  // نباید دوباره ارسال شوند. فرم واقعی صفحه کپچا فقط فیلدهای hidden خودش را می‌فرستد؛
  // سرور تاریخ/مسیر را در نشست (PHPSESSID) با کلید srvc نگه می‌دارد. ارسال دوباره
  // (به‌ویژه departureTrain) ممکن است رزرو را رد کند (خطای 101).
