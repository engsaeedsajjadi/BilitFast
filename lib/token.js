...
function getKey() {
  const hex = getRequiredEnv('BILITFAST_TOKEN_KEY', {
    devFallback: crypto.createHash('sha256').update(DEFAULT_DEV_KEY).digest('hex'),
    validate: (v) => /^[0-9a-fA-F]{64}$/.test(String(v || '').trim()),
  });
