...
async function verifySession(cookies) {
  ...
  // اگر هنوز فرم ورود نشان داده می‌شود، یعنی نشست وارد نشده است.
  let text = '';
  try { text = await resp.text(); } catch (e) { text = ''; }
  const loginFormShown = /name=["']?pass["']?|گذرواژه|UserAut\.php|Login\/process/i.test(text);
...
async function login(username, password) {
  ...
  if (marker) {
    ...
    const stillLogin = /ورود به سامانه|شناسه:|گذرواژه|UserAut|Login\/process/i.test(text);
