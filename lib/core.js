...
async function searchOnce({ fields, passengers, cookies }) {
  ...
  let note = null;
  if (trains.length === 0 && (htmlText.includes('موجودی') || htmlText.includes('ظرفیت کوپه'))) {
    note = 'قطارها یافت شدند اما موجودی در جستجوی بدون ورود صفر است؛ برای دیدن ظرفیت واقعی، کوکی‌های ورود خود را در صفحه «ورود» ثبت کنید.';
  }

  return { ok: true, trains, searchData, note };
}
