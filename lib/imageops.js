...
/**
 * واریانس دایره‌ای «جهت ضربه» (برگرفته از SWT): برای هر پیکسل جوهر، جهتِ محورِ
 * اصلی جوهر در همسایگی ۳×۳ محاسبه می‌شود (جهت در امتداد ضربه). اگر همهٔ پیکسل‌ها
 * هم‌جهت باشند (خط راست) واریانس ≈ ۰ است؛ کاراکترها به‌خاطر انحنا/شاخه‌ها واریانس
 * بالاتری دارند. خروجی در بازهٔ [0,1].
 */
function strokeDirectionVariance(bin) {
  const W = bin.width, H = bin.height;
  let sx = 0, sy = 0, n = 0;
  for (let y = 1; y < H - 1; y++) {
    for (let x = 1; x < W - 1; x++) {
      if (bin.data[y * W + x] !== 0) continue;
      let mxx = 0, myy = 0, mxy = 0, cnt = 0;
      for (let dy = -1; dy <= 1; dy++) {
        for (let dx = -1; dx <= 1; dx++) {
          if (bin.data[(y + dy) * W + (x + dx)] !== 0) continue;
          mxx += dx * dx; myy += dy * dy; mxy += dx * dy; cnt++;
        }
      }
      if (cnt < 2) continue;
      const theta = 0.5 * Math.atan2(2 * mxy, mxx - myy);
      sx += Math.cos(2 * theta); sy += Math.sin(2 * theta); n++;
    }
  }
  if (n < 4) return 1;
  const R = Math.hypot(sx, sy) / n; // ۱ = کاملاً هم‌جهت (خط راست)
  return 1 - R;
}

/**
 * آمار هندسی-ضربه‌ای یک مؤلفهٔ دودویی (پایهٔ تفکیک «خط نویز» از «کاراکتر»):
 *  w       عرض ضربه (۲×میانهٔ تبدیل فاصله)
 *  len     طول تقریبی مرکزخط = مساحت/عرض ضربه
 *  ribbon  نسبت «نواری بودن» = طول مرکزخط/قطر جعبه — خط راست ≈ ۱ یا کمتر،
 *          کاراکترِ جمع‌شده (مثل 8 یا 3) به‌وضوح بزرگ‌تر
 *  dirVar  واریانس جهت ضربه — خط راست ≈ ۰
 *  orient  زاویهٔ محور اصلی (تانسور اینرسی) به درجه [0,90): ۰=افقی، ۹۰=عمودی؛
 *          کاراکترها تقریباً عمودی/افقی رسم می‌شوند ولی خطوط نویز مورب‌اند.
 */
function strokeStats(bin) {
  const W = bin.width, H = bin.height;
  let area = 0, sx = 0, sy = 0;
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      if (bin.data[y * W + x] === 0) { area++; sx += x; sy += y; }
    }
  }
  const d = distanceTransform(bin);
  const ds = [];
  for (let i = 0; i < W * H; i++) if (bin.data[i] === 0) ds.push(d[i]);
  ds.sort((a, b) => a - b);
  const w = Math.max(2, 2 * (ds[Math.floor(ds.length / 2)] || 1));
  const bw = W, bh = H;
  const diag = Math.hypot(bw, bh) || 1;
  const len = area / w;
  // محور اصلی از تانسور اینرسی
  let orient = 90;
  if (area > 6) {
    ...
    const theta = 0.5 * Math.atan2(2 * mxy, mxx - myy); // رادیان
    orient = Math.abs(theta) * 180 / Math.PI; // [0,90]
    if (orient > 90) orient = 180 - orient;
  }
  return {
    area,
    w,
    ...
    len,
    ribbon: len / diag,
    dirVar: strokeDirectionVariance(bin),
    orient,
  };
}
...
/**
 * تبدیل فاصله (chamfer، دو گذر): برای هر پیکسل جوهر، فاصله شطرنجی تا نزدیک‌ترین
 * پیکسل پس‌زمینه. عرض ضربه محلی ≈ 2*d − 1؛ این پایهٔ «تحلیل عرض ضربه» (SWT) است:
 * بدنهٔ خطوط نویز باریک است (d کوچک) و بدنهٔ کاراکترها پهن (d بزرگ).
 */
function distanceTransform(bin) {
  const W = bin.width, H = bin.height;
  const INF = 1 << 20;
  const d = new Int32Array(W * H);
  for (let i = 0; i < W * H; i++) d[i] = bin.data[i] === 0 ? INF : 0;
  // پیش‌رو
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      const i = y * W + x;
  ...
  // برای پیکسل‌های جوهر، فاصله حداقل ۱ است
  for (let i = 0; i < W * H; i++) if (bin.data[i] === 0 && d[i] < 1) d[i] = 1;
  return d;
}

/**
 * حذف خطوط نویز با «تحلیل عرض ضربه» (SWT):
 *  ۱) هسته = پیکسل‌هایی با عرض ضربه ≥ coreWidth (فقط بدنهٔ کاراکترهای پهن)
 *  ۲) بازسازی مهارشده: گسترش از هسته فقط به پیکسل‌هایی با عرض ≥ allowWidth؛
 *     بدنهٔ خط نازک اجازهٔ عبور نمی‌گیرد، پس خط (حتی خطِ روی کاراکتر) حذف
 *     می‌شود ولی شکل کاراکتر حفظ می‌گردد.
 */
function strokeWidthFilter(bin, { coreWidth = 5, allowWidth = 3 } = {}) {
  const d = distanceTransform(bin);
  const W = bin.width, H = bin.height;
  const coreR = Math.ceil((coreWidth + 1) / 2);
  const allowR = Math.ceil((allowWidth + 1) / 2);
  const core = makeImage(W, H);
  let hasCore = false;
  for (let i = 0; i < W * H; i++) {
    if (bin.data[i] === 0 && d[i] >= coreR) { core.data[i] = 0; hasCore = true; }
  }
  if (!hasCore) return bin; // تصویر یکسره نازک است؛ دست نمی‌زنیم
  let cur = core;
  for (let k = 0; k < 12; k++) {
    const dil = dilate(cur, 1);
    const next = makeImage(W, H);
    let changed = false;
    for (let i = 0; i < W * H; i++) {
      const v = (dil.data[i] === 0 && bin.data[i] === 0 && d[i] >= allowR) ? 0 : 255;
      if (v !== cur.data[i]) changed = true;
      next.data[i] = v;
    }
    cur = next;
    if (!changed) break;
...
/**
 * حذف ساختارهای نازک (خطوط نویز) با «بازسازی مورفولوژیک»:
 *   هسته = فرسایش تصویر (ساختارهای نازک‌تر از پنجره نابود می‌شوند)
 *   بازسازی = گسترشِ مهارشده به هسته تا خود تصویر (شکل کامل کاراکتر برمی‌گردد
 *   اما خطِ بدون هستهر هرگز).
 * این روش شکل کامل کاراکترها را حفظ می‌کند و خطوط را کامل حذف می‌کند —
 * بسیار بهتر از آستانه‌های ضریب پُری.
 */
function removeThinLines(bin, iterations = 1) {
  const core = erode(bin, iterations);
  let hasCore = false;
  for (let i = 0; i < core.data.length; i++) {
    if (core.data[i] === 0) { hasCore = true; break; }
  }
  if (!hasCore) return bin; // همه‌چیز نازک بود؛ دست نمی‌زنیم
  let cur = core;
  for (let k = 0; k < 10; k++) {
    const dil = dilate(cur, 1);
    const next = makeImage(bin.width, bin.height);
    let changed = false;
    for (let i = 0; i < next.data.length; i++) {
      const v = (dil.data[i] === 0 && bin.data[i] === 0) ? 0 : 255;
      if (v !== cur.data[i]) changed = true;
      next.data[i] = v;
    }
    cur = next;
...
module.exports = {
  ...
  erode, dilate, morphOpen, morphClose, removeThinLines,
  distanceTransform, strokeWidthFilter, strokeDirectionVariance, strokeStats,
